...
export default function WorksCard({data}) {
  return (
    <Card sx={{ maxWidth: 345, height: 360, position: 'relative',}}>
      <CardMedia
        sx={{ height: 160 }}
      ...
      <CardContent>
        <Typography gutterBottom variant="h5" component="div">
         {data.title}{data.wip && <Chip label="WIP" color="warning" size="small" sx={{marginLeft: "6px"}}/>}
        </Typography>
        ...
        <Box sx={{ position: 'absolute', bottom: 0 }}>
            <CardActions>
                <Link target="_blank" href={data.link} rel="noreferrer">
                    <Button size="small" disabled={data.wip || !data.linked}>Learn More</Button>
                </Link>
            </CardActions> 
        </Box>
    </Card>
