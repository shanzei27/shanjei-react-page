...
const Projects = () => {
  ...
  return (
    ...
    <Grid item xs={10} sm={9}>
      <Typography variant='h2' sx={{marginBottom: '5px'}}>Projects</Typography>
      <Grid justify="center" container sx={{ width: '100%', padding: "5px" }} spacing={3}>
            {worksConfig.map((item) => (
              <Grid align="center" item xs={12} sm={12} md={6} lg={4} key={item.title}>
              <WorksCard data={item} />
              </Grid>
            ))}
    ...
    <PortfolioHeaderContainer sx={{display: 'flex'}}>
        <Typography variant='h5' sx={{color: "#fff"}}>Portfolio</Typography>
        <ExpandMore
